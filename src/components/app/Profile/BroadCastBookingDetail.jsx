...
export default function BroadCastBookingDetail() {
    ...
    const { requestDetails, requestisLoading } = useSelector((state) => state.user);
                                ...
                                <div className="p-6">
                                    ...
                                    <div className="flex justify-between items-start mb-0">
                                        <div className="flex items-center justify-between w-full">
                                            ...
                                            <div className="mt-0">
                                                <p className={`text-xs font-semibold ${data?.status === 'pending' ? 'text-white bg-yellow-300 rounded-xl capitalize w-[5em] flex text-center justify-center' : 'text-green-500'}`}>{data?.status}</p>
                                            </div>
                                        </div>
                                    ...
                                    <div className="flex justify-between items-center">
                                        <div className="flex gap-12">
                                            <div>
                                                ...
                                                <p className="text-xl font-semibold text-gray-900">${data?.total_payment}</p>
                                            </div>
                                            <div>
                                                <p className="text-sm text-gray-500 mb-1">Preferred Date & Time</p>
                                                <p className="text-lg font-medium text-gray-900">{data?.date} | {data?.time}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
