    ...
    <div>
      ...
      <div className="h-full px-10 lg:px-40  -mt-80 bottom-0 items-center gap-3 ">
        ...
        <div className="bg-[#F9FAFA] shadow-lg flex flex-col gap-3 mb-48 rounded-[8px] p-4 mt-3">
          {menuItems
            // .filter(item => !(role === "service_provider" && item.label === "Subscription")) // hide for user
            .map((item, index) => (
              <div
                key={index}
                onClick={() =>
      ...
      {activeModal == "Report An Issue" && (
        <ReportAnIssueModal isOpen={isOpen} setIsOpen={setIsOpen} />
      )}
